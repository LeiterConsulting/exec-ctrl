...
1. Inspect the repository and the current request. If the prompt mentions the `exec-ctrl` repo only as a method reference and no other target repo link is present, treat the current repository as the target workspace and the `exec-ctrl` URL as the method source. If the request is actually to improve the `exec-ctrl` repo itself as a governed method repository, switch to the self-hosting method-evolution path instead of generic bootstrap.
...
- state whether the governed target is the current workspace, an external repo, or the `exec-ctrl` repo itself under self-hosting rules
